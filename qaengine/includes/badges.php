...
function ae_register_post_type() {
	register_post_type('qa_level', array(
		'labels' => array(
	...
	$qa_pack = new AE_Pack('qa_level', array());
	global $ae_post_factory;
	$ae_post_factory->set('qa_level', $qa_pack);
}

/**
 * Add qa_point_text meta
 * @param object $args
 * @return object $args
 * @author tatthien
 * @version 2.0
 */
function qa_badges_convert($args) {
	$args['qa_point_text'] = sprintf(__( '%d points' , ET_DOMAIN ), $args['qa_badge_point'] );
	return $args;
}
add_filter('ae_pre_insert_qa_level', 'qa_badges_convert');
...
function qa_get_badge_point() {
	$badge	=	QA_BadgeOptions::get_instance();
	$badge->pos_badges = isset($badge->pos_badges) ? $badge->pos_badges : array();
	$badge->neg_badges = isset($badge->neg_badges) ? $badge->neg_badges : array();
 ...
 * @package qaengine
*/
if ( ! function_exists ( 'qa_user_badge' ) ) {
...
function qa_get_user_caps() {
	...
	if( $user_ID ) {
		...
		foreach ( $privileges as $privi => $point ) {
			if($privi == 'add_comment' && !empty($post) && $post->post_author == $user_ID){
				$cap[$privi]	=	true;
			}
