(function (Views, Models, $, Backbone) {
	Views.MobileSingleAnswer	= Backbone.View.extend({
		el : 'body.single-answer ',
		events : {
		...
		openReportModal : function(event){
			event.preventDefault();
			if( typeof this.modalReport == "undefined"){
				this.modalReport = new Views.ReportModal({
					el: $("#reportFormModal")
				});
			}
			var model = new Models.Post(currentQuestion);
			this.modalReport.setModel(model)
			this.modalReport.openModal();
		}
	});
